...
if [[ $P4_NAME == "switch" ]]; then
  ...
  if [[ ! -d $TEST_DIR ]]; then
    TEST_DIR=`find $SDE -type d -path "*p4-examples*/p4_16_programs/$P4_NAME"`
  fi
fi
...
sudo env "PATH=$PATH" "LD_LIBRARY_PATH=$LD_LIBRARY_PATH" $DBG tofino-model \
	...
	$NO_LOG \
	--chip-type $CHIPTYPE \
	-f $PORTINFO $PORTMONITOR $LOG_DIR $JSON_LOGS_ENABLE $PKT_LOG_LEN $INT_PORT_LOOP $USE_PCIE_VETH $DOD_TEST_MODE $TIME_DISABLE $@
