...
def analyze_flow_completion():
    ...
    with open(run_folder_path + '/flow_completion.csv.log') as file:
        ...
        for row in reader:
            ...
            src_port.append(float(row[9]))
            dst_port.append(float(row[10]))

            if len(row) != 11:
                print("Invalid row: ", row)
                exit()

        # Compute statistics if any of the flows finished transmission
        if lines_file == 0:
            print("None of the flows completed its transmission. Thus, skipping flow-completion statistics...")
        else:
