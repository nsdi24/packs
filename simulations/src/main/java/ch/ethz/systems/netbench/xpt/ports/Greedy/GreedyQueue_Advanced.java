...
public class GreedyQueue_Advanced implements Queue {
    ...
    public GreedyQueue_Advanced(long numQueues, long perQueueCapacity, NetworkDevice ownNetworkDevice, String initialization, String fixQueueBounds){
        ...
        } else if (initialization.equals("exponential")) {
            this.rank_bound = 100; // Needs to be bigger than the initial maximum queue bound
            queueBounds.put(0, 0);
            queueBounds.put(1, 3);
            queueBounds.put(2, 7);
            queueBounds.put(3, 11);
            queueBounds.put(4, 16);
            queueBounds.put(5, 23);
            queueBounds.put(6, 32);
            queueBounds.put(7, 48);
        } else if (initialization.equals("inverse_exponential")) {
            this.rank_bound = 100; // Needs to be bigger than the initial maximum queue bound
            queueBounds.put(0, 0);
            queueBounds.put(1, 52);
            queueBounds.put(2, 68);
            queueBounds.put(3, 77);
            queueBounds.put(4, 84);
            queueBounds.put(5, 89);
            queueBounds.put(6, 93);
            queueBounds.put(7, 97);
        } else if (initialization.equals("convex")) {
            this.rank_bound = 100; // Needs to be bigger than the initial maximum queue bound
            queueBounds.put(0, 0);
            queueBounds.put(1, 3);
            queueBounds.put(2, 6);
            queueBounds.put(3, 11);
            queueBounds.put(4, 23);
            queueBounds.put(5, 81);
            queueBounds.put(6, 86);
            queueBounds.put(7, 89);
        } else if (initialization.equals("minmax")) {
            this.rank_bound = 100; // Needs to be bigger than the initial maximum queue bound
            queueBounds.put(0, 0);
            queueBounds.put(1, 29);
            queueBounds.put(2, 34);
            queueBounds.put(3, 37);
            queueBounds.put(4, 39);
            queueBounds.put(5, 41);
            queueBounds.put(6, 43);
            queueBounds.put(7, 46);
        } else if (initialization.equals("poisson")) {
            this.rank_bound = 100; // Needs to be bigger than the initial maximum queue bound
            queueBounds.put(0, 0);
            queueBounds.put(1, 43);
            queueBounds.put(2, 46);
            queueBounds.put(3, 48);
            queueBounds.put(4, 50);
            queueBounds.put(5, 52);
            queueBounds.put(6, 44);
            queueBounds.put(7, 58);
        ...
        for (int q=queueList.size()-1; q>=0; q--){
            ...
            if ((currentQueueBound <= rank) || q==0) {
                ...
                if (!result){
                    ...
                    int rank_perceived = 0;
                    if (packet_maxrank.isTCP()){
                        rank_perceived = (int)((FullExtTcpPacket)packet_maxrank).getPriority();
                    } else {
                        rank_perceived = (int)((FullExtUdpPacket)packet_maxrank).getPriority();
                    }
                ...
                if (SimulationLogger.hasInversionsTrackingEnabled()) {
                    ...
                    for (int i = 0; i <= queueList.size() - 1; i++) {
                        ...
                        if (currentQueue.length > 0) {
                            ...
                            Packet currentMin = (Packet) currentQueue[0];
                            if (currentMin.isTCP()){
                                if ((int)((FullExtTcpPacket)currentMin).getPriority() < rankSmallest){
                                    rankSmallest = (int)((FullExtTcpPacket)currentMin).getPriority();
                                }
                            } else {
                                if ((int)((FullExtUdpPacket)currentMin).getPriority() < rankSmallest){
                                    rankSmallest = (int)((FullExtUdpPacket)currentMin).getPriority();
                                }
                            }
                        }
                    }
