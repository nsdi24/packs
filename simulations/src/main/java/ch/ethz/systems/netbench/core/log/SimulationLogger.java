...
import ch.ethz.systems.netbench.xpt.aifotransport.AIFOFlowLogger;
...
public class SimulationLogger {
    ...
    // AIFO Extension
    private static BufferedWriter writerWindowLengthFile;
    private static BufferedWriter writerRankReceived;
    ...
    // PACKS Extension
    private static BufferedWriter writerDropsTracking;
    private static boolean dropsTrackingEnabled;
    private static BufferedWriter writerPacketsTracking;
    private static boolean packetsTrackingEnabled;
     ...
     */
    public static void registerFlowLogger(FlowLogger logger) {
    ...
    public static void open(NBProperties tempRunConfiguration) {
        ...
        if (tempRunConfiguration != null) {
            ...
            // PACKS: Enabling logs
            dropsTrackingEnabled = tempRunConfiguration.getBooleanPropertyWithDefault("enable_drops_tracking", false);
            packetsTrackingEnabled = tempRunConfiguration.getBooleanPropertyWithDefault("enable_packets_tracking", false);
        }
        ...
        try {
            ...
            // PACKS log writers
            if (packetsTrackingEnabled) {
                    writerPacketsTracking = openWriter("packets_tracking.csv.log");
            }
            if (dropsTrackingEnabled){
            writerDropsTracking = openWriter("drops_tracking.csv.log");
            }

            // AIFO log writers
            writerWindowLengthFile = openWriter("window_length.log");
            writerRankReceived = openWriter("rank_at_10.log");
            writerRankReceived.write("rankx" + "   " + "flowid" + "   "+ "timex"+"\n");
    ...
    public static void close() {
        ...
        try {
            ...
            statisticCounters.clear(); // Added by AIFO's developers
            ...
            // PACKS: Close log files
            if (packetsTrackingEnabled) {
                writerPacketsTracking.close();
            }
            if (dropsTrackingEnabled) {
                writerDropsTracking.close();
            }

	    // AIFO: Close log files
            writerWindowLengthFile.close();
            writerRankReceived.close();
    ...
    /* PACKS Logging */
    public static void logPacket(String text) {
        try {
            writerPacketsTracking.write(text + "\n");
        } catch (IOException e) {
            throw new LogFailureException(e);
        }
    }

    public static void logDropsPerRank(int id, int rank, long drop) {
        try {
            writerDropsTracking.write(id + "," + rank + "," + drop + "\n");
        } catch (IOException e) {
            throw new LogFailureException(e);
        }
    }
    ...
    /* AIFO Logging */
    public static void logWindowLength(int windowLength) {
        try {
            writerWindowLengthFile.write(windowLength + "\n");
        } catch (IOException e) {
            throw new LogFailureException(e);
        }
    }
    ...
    /**
     * log the rank received by server 10
     * @param rank
     */
    public static void logRankReceived(long flowid, long rank, long time) {
        try {
            writerRankReceived.write(rank + "   " + flowid + "   "+ time + "\n");
        } catch (IOException e) {
            throw new LogFailureException(e);
        }
    }
    ...
    private static void logFlowSummary() {
        try {
            ...
            for (FlowLogger logger : flowLoggers) {
                ...
                if (logger instanceof AIFOFlowLogger){
                    ...
                    writerFlowCompletionCsvFile.write(
                            logger.getFlowId() + "," +
                                    ...
                                    (logger.isCompleted() ? "TRUE" : "FALSE") + "," + ((AIFOFlowLogger)logger).getSourcePort() + "," + ((AIFOFlowLogger)logger).getDestinationPort() + "\n"
                    );
                } else {
                    // flowId, sourceId, targetId, sentBytes, totalBytes, flowStartTime, flowEndTime, flowDuration, isCompleted
                    writerFlowCompletionCsvFile.write(
                            logger.getFlowId() + "," +
                                    ...
                                    (logger.isCompleted() ? logger.getFlowEndTime() : Simulator.getCurrentTime()) + "," +
                                    (logger.isCompleted() ? (logger.getFlowEndTime() - logger.getFlowStartTime()) : (Simulator.getCurrentTime() - logger.getFlowStartTime())) + "," +
                                    (logger.isCompleted() ? "TRUE" : "FALSE") + "," + "80" + "," + "80" + "\n"
                    );
                }
    ...
    private static void copyFileToRunFolder(String fileName) {
        ...
        MainFromProperties.runCommand("cp " + fileName + " " + getRunFolderFull());
    }
    ...
    public static void copyFileToRunFolder(String fileName, String newFileName) {
        ...
        MainFromProperties.runCommand("cp " + fileName + " " + getRunFolderFull() + "/" + newFileName);
    }
    ...
    public static boolean hasPacketsTrackingEnabled() { return packetsTrackingEnabled; }

    public static boolean hasDropsTrackingEnabled() { return dropsTrackingEnabled; }
